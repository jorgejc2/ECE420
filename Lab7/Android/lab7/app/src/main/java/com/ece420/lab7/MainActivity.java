...
public class MainActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        if(tracking_flag == -1){
            ...
            long timer = Core.getTickCount();
            boolean read_success = myTracker.update(mGray, myROI);
            int fps = (int)(Core.getTickFrequency() / (Core.getTickCount() - timer));

            /* parameters to set for placing text */
            Point text_position = new Point(40.0,80.0);
            int font_size = 1;
            Scalar font_Color = new Scalar(255,0,0);
            int font_weight = 2;
...
//                        4);
                ...
                Imgproc.putText(mRgba, "Tracking failure occurred!", text_position, Core.FONT_HERSHEY_COMPLEX, font_size, font_Color, font_weight);
            }

            /* display FPS */
            font_Color.set(new double []{0,255,0});
            text_position.y = 120;
            Imgproc.putText(mRgba, String.format("FPS %d", fps), text_position, Core.FONT_HERSHEY_COMPLEX, font_size, font_Color, font_weight);
