...
public class CameraActivity extends AppCompatActivity implements SurfaceHolder.Callback{
    ...
    public byte[] histEq(byte[] data, int width, int height){
        ...
        int L = 256; // number of bins for uint8

        int[] histo = new int[L]; // For uint8 images

        /* fill in histogram */
        for (int i = 0; i < size; i++) {
            histo[data[i] & 0x00FF]++; // AND with 0x00FF to handle as uint8
        }

        /* convert to cdf and find cdf_min */
        int cdf_min = 0;
        ...
        int helper = 0;
        for (int i = 0; i < L; i++) {
            helper = (int)(((1.0*histo[i] - cdf_min) / (size - 1))*(L-1));
            histo[i] = helper;
        }

        for (int i = 0; i < size; i++) {
            histeqData[i] = (byte)histo[data[i] & 0x00FF];
        }
    ...
    public int[] conv2(byte[] data, int width, int height, double kernel[][]){
        ...
        int half_width = (int)(kernel_width / 2);
        int sum;
        int curr_pixel_i;
        int curr_pixel_j;
        int conv_data_idx;
            ...
            for (int j = 0; j < width; j++) {
                ...
                for (int h = 0; h < kernel_width; h++) {
                    for (int w = 0; w < kernel_width; w++) {
                        curr_pixel_i = i - half_width + h;
                        curr_pixel_j = j - half_width + w;
                        curr_pixel_idx = curr_pixel_i * width + curr_pixel_j;
                        if (curr_pixel_i >=0 && curr_pixel_i < height && curr_pixel_j >= 0 && curr_pixel_j < width){
                            sum += kernel[h][w] * data[curr_pixel_idx];
                        }
                    }
                }
                conv_data_idx = i * width + j;
                convData[conv_data_idx] = sum;
            }
        }
