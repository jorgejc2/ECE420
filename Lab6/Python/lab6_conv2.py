...
def conv2(pic,kernel):
    ...
    half_width = kernel.shape[0] // 2
    
    # for every layer in the image
    for l in range(pic.shape[2]):
        # for every row 
        for i in range(pic.shape[0]):
            # for every column
            for j in range(pic.shape[1]):
                sum = 0
                # for every row in the kernel
                for h in range(kernel_width):
                    ...
                    for w in range(kernel_width):
                        ...
                        if (curr_pixel_i > -1 and curr_pixel_i < pic.shape[0]) and (curr_pixel_j > -1 and curr_pixel_j < pic.shape[1]):
                            sum += kernel[h, w] * pic[curr_pixel_i, curr_pixel_j, l]
                pic_conv[i,j,l] = sum/255
